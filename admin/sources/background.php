...
function save_gioithieu(){
	...
	if(count($row_item)>0){
		...
		if($d->update($data))
			...
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=background&act=capnhat&type=".$_GET['type']);
	}else{
		if($photo = upload_image("file", _img_type,_upload_hinhanh,$file_name)){
			$data['photo'] = $photo;
		}
		...
		{			
			...
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=background&act=capnhat&type=".$_GET['type']);
	}
}

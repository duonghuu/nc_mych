...
function get_sub(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_sub&type=".$_GET['type']);
	
	$sql = "select * from #_baiviet_sub where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=baiviet&act=man_sub&type=".$_GET['type']);
	$item = $d->fetch_array();
}
...
function save_sub(){
	...
	if($id){
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=baiviet&act=man_sub&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=baiviet&act=man_sub&type=".$_GET['type']);
	}else{
		if($photo = upload_image("file", 'jpg|png|gif|PNG|GIF|JPG|JPEG|jpeg', _upload_baiviet,$file_name)){
			$data['photo'] = $photo;
			$data['thumb'] = create_thumb($data['photo'], _width_thumb, _height_thumb, _upload_baiviet,$file_name,_style_thumb);
		...
		$d->setTable('baiviet_sub');
		if($d->insert($data))
			redirect("index.php?com=baiviet&act=man_sub&type=".$_GET['type']);
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=baiviet&act=man_sub&type=".$_GET['type']);
	}
}
...
function delete_sub(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=baiviet&act=man_sub&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
	} elseif (isset($_GET['listid'])==true){
		$listid = explode(",",$_GET['listid']); 
		for ($i=0 ; $i<count($listid) ; $i++){
			$idTin=$listid[$i]; 
		...
		transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_sub&curPage=".$_REQUEST['curPage']."&type=".$_GET['type']);
	}
}
#====================================

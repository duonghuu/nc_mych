...
switch($act){
	...
	if($_GET['type']=='gioithieu'){
		$title_main = 'giới thiệu';
	}elseif($_GET['type']=='tuyendung'){
...
function save_info(){
	...
	if(count($row_item )>0){
		...
		if($photo = upload_image("img", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_info,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 295, 195, _upload_info,$file_name,1);		
			$d->setTable('baiviet');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){

...
if(!empty($_POST)){
   ...
   if(isset($_POST) && $_POST['xacnhan']!='')
   {
		...
		<table border="1" bordercolor="#0099CC" align="center" cellpadding="5px" cellspacing="5px" width="100%">';
			...
			{
            	$body.='<tr bgcolor="#16AAB8"><td>Thứ tự</td><td>Hình ảnh</td><td>Tên</td><td>Size</td><td>Giá</td><td>Số lượng</td><td>Tổng giá</td></tr>';
				...
				for($i=0;$i<$max;$i++){
            		...
            		$body.='<td>'.$psize.'</td>';
                    $body.='<td>'.number_format(get_gia($pid,$mid),0, ',', '.').'&nbsp;VND</td>';
                    $body.='<td>'.$q.'</td>';                 
                    $body.='<td>'.number_format(get_gia($pid,$mid)*$q,0, ',', '.') .'&nbsp;VND</td>
                    </tr>';
				}
  ...
  <tr><td colspan="2" align="left">
			...
			if($mail->Send()){
				...
				$url_cancel='http://'.$config_url.'/huy-thanh-toan.htm';
				$url_detail=='';
			...
			else transfer("Xin lỗi quý khách.<br>Hệ thống bị lỗi, xin quý khách thử lại.", "thanh-toan.htm");
		}
		}
